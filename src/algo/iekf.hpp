...
namespace zjloc
{
    ...
    public:
                                  ...
                                  const VecT &gravity = VecT(0, 0, -9.8))
        {
            ...
            // cov_.template block<3, 3>(0, 0) = 0.1 * math::kDEG2RAD * Mat3T::Identity(); //  for R
        }
        ...
        {
            // std::cout << __FUNCTION__ << ": " << current_time_ << std::endl;
            return NavStateT(current_time_, R_, p_, v_, bg_, ba_, g_, cov_);
        }
        ...
        void SetX(NavStated &x)
        {
            std::cout << "---------------------" << std::endl;
            std::cout << current_time_ << "--->" << x.timestamp_ << std::endl;
            std::cout << p_.transpose() << "--->" << x.p_.transpose() << std::endl;
            std::cout << v_.transpose() << "--->" << x.v_.transpose() << std::endl;
            std::cout << bg_.transpose() << "--->" << x.bg_.transpose() << std::endl;
            std::cout << ba_.transpose() << "--->" << x.ba_.transpose() << std::endl;
            std::cout << g_.transpose() << "--->" << x.g_.transpose() << std::endl;
    ...
    private:
        void BuildNoise(const Options &options)
        {
            ...
            // set Q
            Q_.diagonal() << et2, et2, et2, 0, 0, 0, ev2, ev2, ev2, eg2, eg2, eg2, ea2, ea2, ea2, 0, 0, 0;
        }
    ...
    bool IESKF<S>::Predict(const IMU &imu)
    {
        ...
        //  r   p   v   bg  ba  g
        F.template block<3, 3>(0, 0) = SO3::exp(-(imu.gyro_ - bg_) * dt).matrix();    //  dr/dr
        F.template block<3, 3>(0, 9) = -Mat3T::Identity() * dt;                       //   dr/dbg
        F.template block<3, 3>(3, 3) = Mat3T::Identity();                             //  dp/dp
        F.template block<3, 3>(3, 6) = Mat3T::Identity() * dt;                        //  dp/dv
        F.template block<3, 3>(6, 0) = -R_.matrix() * SO3::hat(imu.acce_ - ba_) * dt; //  dv/dr
        F.template block<3, 3>(6, 6) = Mat3T::Identity();                             //   dv/dv
        F.template block<3, 3>(6, 12) = -R_.matrix() * dt;                            //  dv/dba
        F.template block<3, 3>(6, 15) = Mat3T::Identity() * dt;                       //  dv/dg
    ...
    bool IESKF<S>::UpdateUsingCustomObserve(IESKF::CustomObsFunc obs)
    {
        ...
        for (int iter = 0; iter < options_.num_iterations_; ++iter)
        {
            ...
            J.template block<3, 3>(0, 0) = Mat3T::Identity() - 0.5 * SO3::hat((R_.inverse() * start_R).log());
        ...
        // project P  对P阵进行投影，参考式(3.59)
        Mat18T J = Mat18T::Identity();
        Vec3d dtheta = (R_.inverse() * start_R).log();
        J.template block<3, 3>(0, 0) = Mat3T::Identity() - 0.5 * SO3::hat(dtheta);
