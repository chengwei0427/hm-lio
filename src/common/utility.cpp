...
void transformPoint(point3D &point_temp, Eigen::Quaterniond &q_end,
                    Eigen::Vector3d &t_end, Eigen::Matrix3d &R_imu_lidar, Eigen::Vector3d &t_imu_lidar)
{
     Eigen::Vector3d t;
     Eigen::Matrix3d R;
...
void transformPoint(point3D &point_temp, SE3 &pose, Eigen::Matrix3d &R_imu_lidar, Eigen::Vector3d &t_imu_lidar)
{
     point_temp.point = pose.so3().matrix() * (R_imu_lidar * point_temp.raw_point + t_imu_lidar) + pose.translation();
     // point_temp.point = pose.so3().matrix() * point_temp.raw_point + pose.translation();
}